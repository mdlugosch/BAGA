...
namespace Model
{
    [CustomValidation(typeof(Trip),"TripDateValidatior")]
    ...
    public class Trip : IValidatableObject
    {
        ...
        public static ValidationResult TripDateValidatior(Trip trip, ValidationContext validateContext)
        {
        ...
        public static ValidationResult TripCostInDescriptionValidator(
            Trip trip, ValidationContext validationContext)
        {
            if(trip.CostUSD > 0)
            {
                if(trip.Description
                ...
                {
                    return new ValidationResult("Description cannot contain trip cost",
                        new[] { "Destination" });
                }
            }
            return ValidationResult.Success;
